(function() {
    ...
    class AutoCheck
    {
        ...
        {
            /*
			1. 在js内部，整数和浮点数是同样的储存方法，所以 3 和 3.0 被视为同一个值，这也是为什么Math.max(3.0,2)得到的结果是3，而不是3.0
			2. 解决方法，此处选择较为方便的 Number.isInteger()
				Number.isInteger不会自动类型转换，示例:
					Number.isInteger(25)   // true
					Number.isInteger(25.0) // true
					Number.isInteger(25.1) // false
					Number.isInteger("15") // false
					Number.isInteger(true) // false
			但是由于此项目中得到正则匹配到的数字字符串经过了Math.max(),Math.min()的运算，
			得到的就是一个数字类型的值，所以可以使用此方法。
			*/
